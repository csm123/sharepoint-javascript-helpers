...
SJH.Utils.updateFormDigest = function(site) {
  var formDigestRefreshInterval = window._spFormDigestRefreshInterval || 1440000;
  if (site) {
    UpdateFormDigest(site, formDigestRefreshInterval);
  }
};
...
SJH.executeQueryAsync = function(context, resolve, reject, returnValue) {
  context.executeQueryAsync(
    ...
    function(sender, args) { 
      SJH.error(sender, args, reject);
    });
};

SJH.addListItem = function (options) {
  return new RSVP.Promise(function(resolve, reject) {
    SJH.Utils.updateFormDigest(options.site);
...
SJH.error = function(sender, args, reject) {
